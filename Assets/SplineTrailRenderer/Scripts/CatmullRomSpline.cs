...
public class Knot
{
	...
	public Knot(Vector3 position , float height = -1f )
	{
...
public class CatmullRomSpline
{
    ...
    public float GetHeight( Marker marker)
    {
        float height = 0;

        if (NbSegments == 0) return height;

        SubKnot[] subKnots = GetSegmentSubKnots(marker.segmentIndex);

        height = Mathf.Lerp(subKnots[marker.subKnotAIndex].height,
            subKnots[marker.subKnotBIndex].height, marker.lerpRatio);
    ...
    private float GetHeightOnSegment( int segmentIndex, float t )
    {
        return knots[segmentIndex].height;
    }
